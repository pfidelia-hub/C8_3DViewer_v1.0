...
void MainWindow::on_DefSettings_clicked() {
...
//        on_pushButtonVertApply_clicked();
        ...
        LineColor->setRgb(clr[0].toInt(), clr[1].toInt(), clr[2].toInt(), clr[3].toInt());
        clr.clear();
        str = t_stream.readLine();
        clr = str.split(' ');
        BackColor->setRgb(clr[0].toInt(), clr[1].toInt(), clr[2].toInt(), clr[3].toInt());
        clr.clear();
        str = t_stream.readLine();
        clr = str.split(' ');
        VerColor->setRgb(clr[0].toInt(),clr[1].toInt(),clr[2].toInt(),clr[3].toInt());
        file.close();
    }
...
void MainWindow::on_pushButton_Screenshoot_clicked() {
    ...
    if (!ui->openGLWidget->grabFramebuffer().save(pathToSave + "/" + curDateStr + curTimeStr + ".JPG",
                                                  NULL,
                                                  100) && pathToSave.length()) {
        QMessageBox::warning(this, "File could not be saved", "Wrong", QMessageBox::Ok);
    } else {
        ui->openGLWidget->grabFramebuffer().save(pathToSave + "/" + curDateStr + curTimeStr + ".BMP",
                                                 NULL,
                                                 100);
    }
}
